...
namespace CodeGenerator
{
    public partial class CTable : UserControl
    {
        ...
        private void _DataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (_DataGridView.Columns[e.ColumnIndex].Name == "_Display")
            {
                Column c = (Column)_DataGridView.Rows[e.RowIndex].Tag;
                var display = _DataGridView.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
                Utils.SetDescription(c, Utils.EP_IsDisplay, display.ToString());
            }
        }
