...
namespace CodeGenerator.Components.DAL
{
	...
	/// </summary>
	public static class Gen_OB_View
	{
...
");
			...
			/// <summary>
			/// 根据传入的 TSQL WHERE 部分的表达式对象，查找返回一行数据。未找到则返回 null
			/// TSQL 格式： SELECT TOP 1 * FROM [" + Utils.GetEscapeSqlObjectName(v.Schema) + @"].[" + Utils.GetEscapeSqlObjectName(v.Name) + @"] WHERE {where}
			/// </summary>
			[System.ComponentModel.DataObjectMethodAttribute(System.ComponentModel.DataObjectMethodType.Select)]
...
");
			...
			/// <summary>
			/// 返回一个表的多行数据只含部分字段，传入 TSQL WHERE 部分
...
");
			...
			/// <summary>
			/// 根据传入的外键s，查找返回一个表的多行数据的部分字段，带额外条件，排序，限定行数
			/// </summary>
			[System.ComponentModel.DataObjectMethodAttribute(System.ComponentModel.DataObjectMethodType.Select)]
			public static OO." + tn + @"Collection SelectAllPart_By_" + s + @"(");
							for (int i = 0; i < fk.Columns.Count; i++)
