...
{
    public class Gen_View_Select_Custom : IGenComponent
    {
        #region Init

        public Gen_View_Select_Custom()
        {
            this._properties.Add(GenProperties.Name, "Gen_View_Select_Custom");
            this._properties.Add(GenProperties.Caption, "查询单条(带条件)");
            this._properties.Add(GenProperties.Group, "生成过程脚本_SELECT类");
            this._properties.Add(GenProperties.Tips, "生成根据条件查询视图单条数据的存储过程脚本");
        }
...
EXEC sys.sp_addextendedproperty @name=N'CodeGenSettings_IsSingleLineResult', @value=N'True' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Select_Custom'
EXEC sys.sp_addextendedproperty @name=N'CodeGenSettings_ResultType', @value=N'" + t.ToString() + @"' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Select_Custom'
