...
");
    ...
    BEGIN
        RAISERROR ('" + t.Schema + @"." + t.Name + @".Delete|Required.Original_" + c.Name + @" Original_" + cn + @"不能为空', 11, 1);
        RETURN -1;
    END;
");
			}
...
根据主键值删除一行数据' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Delete'
