...
{
    public class Gen_Table_Update : IGenComponent
    {
        #region Init

        public Gen_Table_Update()
        {
            this._properties.Add(GenProperties.Name, "Gen_Table_Update");
            this._properties.Add(GenProperties.Caption, "更新单条");
            this._properties.Add(GenProperties.Group, "生成过程脚本_UPDATE类");
            this._properties.Add(GenProperties.Tips, "生成更新单条数据的存储过程");
        }
            ...
            SELECT 1 FROM [" + Utils.GetEscapeSqlObjectName(t.Schema) + @"].[" + Utils.GetEscapeSqlObjectName(t.Name) + @"]
             WHERE ");
                for (int i = 0; i < pks.Count; i++)
                {
                    ...
                    string cc = Utils.GetCaption(c);
                    if (i > 0) sb.Append(@" AND ");
                    if (c.Identity) sb.Append(@"[" + Utils.GetEscapeSqlObjectName(c.Name) + @"] = @Original_" + cn);
...
根据主键的值定位一行数据并修改' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Update'
