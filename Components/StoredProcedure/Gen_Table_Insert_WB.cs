...
{
	public class Gen_Table_Insert_WB : IGenComponent
	{
		#region Init

		public Gen_Table_Insert_WB()
		{
			this._properties.Add(GenProperties.Name, "Gen_Table_Insert_WB");
			this._properties.Add(GenProperties.Caption, "插入单条（带系统值参回写）");
		...
		public GenResult Gen(params object[] sqlElements)
		{
			...
			Column ic = Utils.GetIdentityColumn(t);
			string icn = (ic == null ? "" : Utils.GetEscapeName(ic));
...
CREATE PROCEDURE [" + Utils.GetEscapeSqlObjectName(t.Schema) + @"].[usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Insert] (");
			...
			foreach (Column c in wcs)
			{
				...
				sb.Append(@"
    " + (isFirst ? "  " : ", ") + Utils.FormatString("@" + cn, Utils.GetParmDeclareStr(c), "= NULL", 40, 40));
				isFirst = false;
			}
			if (ic != null)
			{
				sb.Append(@"
    " + (isFirst ? "  " : ", ") + Utils.FormatString("@" + icn, Utils.GetParmDeclareStr(ic), "= NULL OUTPUT", 40, 40));
			}
			sb.Append(@"
) AS
BEGIN
...
添加一行数据，回写" + icn + @"自增字段值' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_Insert'
