...
{
    public class Gen_Table_InsertPart : IGenComponent
    {
        #region Init

        public Gen_Table_InsertPart()
        {
            this._properties.Add(GenProperties.Name, "Gen_Table_InsertPart");
            this._properties.Add(GenProperties.Caption, "插入单条(部分字段)");
            this._properties.Add(GenProperties.Group, "生成过程脚本_INSERT类");
            this._properties.Add(GenProperties.Tips, "生成插入单条数据(部分字段)的存储过程");
        }
...
EXEC sys.sp_addextendedproperty @name=N'CodeGenSettings_IsSingleLineResult', @value=N'True' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_InsertPart'
EXEC sys.sp_addextendedproperty @name=N'CodeGenSettings_ResultType', @value=N'" + t.ToString() + @"' , @level0type=N'SCHEMA',@level0name=N'" + t.Schema + @"', @level1type=N'PROCEDURE',@level1name=N'usp_" + Utils.GetEscapeSqlObjectName(t.Name) + @"_InsertPart'
